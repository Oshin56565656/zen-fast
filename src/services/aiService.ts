  ...
  try {
    const response = await withTimeout(
      ai.models.generateContent({
        ...
        contents: prompt,
        config: {
          systemInstruction: "You are an expert fasting and fitness coach. Provide data-driven, structured insights based on the user's history and physical profile (age, sex, height, and weight if provided). Be precise about timing relationships. Specifically, recommend the optimal workout time and intensity based on the user's last meal, current fasting state, and body metrics. IMPORTANT: For 'calorieGuess' (intake), you MUST ONLY include calories from meals explicitly logged by the user today. If meals in the logs have non-zero 'calories' values provided, use those exact values. DO NOT estimate or predict meals that haven't been logged yet. For 'caloriesBurned', you MUST always provide a FULL 24-HOUR projection consisting of: 1. Your Basal Metabolic Rate (BMR) for the entire 24-hour day. 2. Your Daily Movement & NEAT (estimated non-exercise activity) for the entire 24-hour day. 3. Calorie burn from workouts explicitly logged today. DO NOT pro-rate BMR or NEAT based on the current time; always provide the full-day total. NEVER hallucinate or predict workouts that are not in the logs. Keep 'reasoning' fields for calorie/burn guesses extremely brief (max 1 short sentence), but ensure the main 'insights' array items are rich and detailed. ALWAYS use 12-hour time format (e.g., 10:00 am) in your responses and include the 'asOfTime' field (matching the user's current local time) in your JSON output.",
